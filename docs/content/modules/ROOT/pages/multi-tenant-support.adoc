...
Redis OM Spring provides building blocks for multi-tenant applications through dynamic index naming, thread-local context management, and flexible index resolution strategies.
These APIs help applications create and manage tenant-specific indexes and key prefixes.

[IMPORTANT]
====
Automatic per-tenant isolation is not currently applied consistently across all repository and `EntityStream` query paths.
Do not rely on `RedisIndexContext` alone to make standard repository operations search only the current tenant's index.
For strict tenant isolation today, scope queries explicitly, for example with an indexed tenant field, a tenant-scoping repository or `EntityStream` facade, and application-level ownership checks.
====
...
try {
    // Context-aware index management APIs can use this tenant context.
} finally {
...
Each thread maintains its own tenant context for APIs that explicitly consult `RedisIndexContext`:
...
Use Spring Expression Language for dynamic index naming and keyspace (key prefix) resolution.
The following example also includes a tenant field for explicit query scoping:
...
This pattern configures tenant-specific index names and key prefixes:
...
[NOTE]
====
Both `indexName` and `keyPrefix` attributes support SpEL expressions.
These expressions are useful for creating tenant-specific indexes and storage prefixes, but automatic tenant routing for all repository and `EntityStream` read paths is not guaranteed.
====
...
Repository operations can be used in multi-tenant applications, but standard repository queries are not automatically scoped by `RedisIndexContext`.
Until tenant-aware query routing is supported across all repository paths, add tenant criteria explicitly or wrap repository access in a tenant-scoping facade:
...
public class ProductService {
    ...
    public Product createProduct(Product product) {
        // Set the tenant field before saving.
        product.setTenantId(tenantService.getCurrentTenant());
        return productRepository.save(product);
    }

    public List<Product> findProductsByCategory(String category) {
        // Include tenant criteria in derived queries or custom queries.
        return productRepository.findByTenantIdAndCategory(tenantService.getCurrentTenant(), category);
    }

    public Page<Product> findProducts(Pageable pageable) {
        // Avoid unscoped findAll(pageable) for tenant-isolated data.
        return productRepository.findByTenantId(tenantService.getCurrentTenant(), pageable);
    }
}
----

The repository needs matching tenant-scoped query methods:
...
Scope `EntityStream` queries explicitly when querying tenant-isolated data:
...
public class AnalyticsService {
    ...
    public List<Product> getPopularProducts(String tenantId) {
        ...
        try {
            return entityStream
                .of(Product.class)
                .filter(Product$.TENANT_ID.eq(tenantId))
...
* **Tenant Isolation**: Enforce tenant criteria consistently on reads and writes
