...
Spring Data Redis (SDR), the library that Redis OM Spring extends, provides mapping of Spring Entities to Redis Hashes. To learn more about Hash Mappings consult the https://docs.spring.io/spring-data/redis/docs/current/reference/html/#redis.repositories[official docs].

Redis OM Spring extends SDR Hash Mapping scheme to take advantage of the https://redis.io/docs/interact/search-and-query/[Redis Query Engine] available as a standard component in Redis 8 or via Redis Stack for older versions.

== Usage

The example on this page is based on an initial Spring Boot application generated with https://start.spring.io as shown below:

image::StartSpringIOHashes.png[]

In addition to the Spring Boot DevTools and Lombok the Maven Redis OM Spring dependency was added to the POM as shown in the xref:setup.adoc[setup section].
...
The entity is annotated with the `@RedisHash` annotation on its type and it has a property named `id` that is annotated with `org.springframework.data.annotation.Id`. These are the minimal requirements needed to map an entity to a Redis Hash.
Those two items are responsible for creating the actual key used to persist the hash. We've also used Lombok's `@Data` and `@RequiredArgsConstructor` to simplify the entity's code.
...
The String `people` is the optional custom keyspace for the persisted Redis Hashes. The key is a combination of the keyspace and the ID value, separated by a colon `:`. In the case above if the ID is `abc123` the resulting key would be `people:abc123`.

Next we define a repository interface, as shown in the following example:

.Basic Redis Enhanced Repository Interface To Persist Person Entities
====
[source,java]
----
...
As our repository extends `RedisEnhancedRepository`, it provides basic CRUD and finder operations, pagination methods, and more.
To enable the repository on a Spring Boot application we can add the annotation `@EnableRedisEnhancedRepositories` to a JavaConfig class, in this example, directly onto the Spring Boot Application class:
...
public class RedisOmHashesApplication {
  @Bean
  CommandLineRunner repositoryTest(PersonRepository repository) {
    return args -> {
      // use your repo here!
    };
  }
...
In the example, we are injecting the `PersonRepository` dependency using Setter-Based Dependency Injection into a `@Bean` annotated method (`repositoryTest`) with a signature returning `CommandLineRunner`. `CommandLineRunner` methods execute after the Spring Boot application has started.
...
// Object-to-Hash mapping is described in detail below
...
For details on Query by Example (QBE), see the xref:qbe.adoc[Query By Example] section.
...
See the Spring Data Redis documentation for `EnableKeyspaceEvents` for possible values.
...
NOTE: `@EnableKeyspaceEvents(shadowCopy = OFF)` disables storage of phantom copies and reduces data size within Redis. `RedisKeyExpiredEvent` will only contain the `id` of the expired key.
