...
Redis Stack implements vector search through RediSearch, which provides high-performance vector indexing and search capabilities. Redis OM Spring makes these capabilities easily accessible through familiar Spring patterns.
...
.Redis OM Spring AI Integration Architecture
[plantuml,ai-architecture,svg]
----
@startuml
package "Application" {
  [Domain Objects] as DO
  [Repositories] as REPO
}

package "Redis OM Spring" {
  [Vector Search API] as VSA
  [Embedding Generation] as EG
  [Index Management] as IM
}

package "Spring AI" {
  [Embedding Models] as EM
}
...
EG --> EM : uses
EM --> ES : may use
IM --> RS : creates/manages indices
VSA --> RS : executes searches
DO --> RJ : stored as
@enduml
----
...
public class Product {
  ...
  @VectorIndexed(dimensions = 1536, distanceMetric = DistanceMetric.COSINE)
...
In this example:
- `@Vectorize` specifies which fields to use for generating the embedding
- `@VectorIndexed` creates a vector index for this field
- When a Product is saved, Redis OM Spring automatically generates an embedding based on the name and description
